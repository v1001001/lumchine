...
lumchine-db模块提供数据库访问服务。

lumchine-core模块提供通用服务。

lumchine-all模块则只是一个包裹模块，几乎没有任何代码。该模块的作用是融合两个spring boot模块
和lumchine-admin模块静态文件到一个单独Spring Boot可执行jar包中。

## 2.1 lumchine数据库

lumchine数据库基于nideshop中的[nideshop.sql](https://github.com/tumobi/nideshop/blob/master/nideshop.sql)数据库，然后在实际开发过程中进行了调整和修改:

* 删除了一些目前不必要的表；
* 删除了表中一些目前不必要的字段；
* 行政区域数据lumchine_region没有采用原nideshop中的数据，而是采用了[Administrative-divisions-of-China](https://github.com/modood/Administrative-divisions-of-China)；
* 表中的某些字段采用JSON存储；
* 表中的日期或时间字段采用DATE、DATETIME；
* 字段的数据类型粗粒度化，例如避免MEDIUMINT，而是INT；
* 表的数据做了清理、调整和补充（假数据）。

lumchine数据库由三个sql文件组成，在lumchine-db文件夹下面的sql文件夹中：

1. lumchine_schema.sql
...
综上，这里lumchine真正必须运行的sql文件是lumchine_table.sql，其他两个sql文件开发者自行决定如何是否使用。
...
这里商品存在商品表（lumchine_goods)，商品属性表（lumchine_goods_attribute），商品规格表（lumchine_goods_specification），商品货品表（lumchine_goods_product）四种表
...
目前准备支持用户普通账号登录和微信登录两种方式，两种登录方式仅仅采用一个lumchine-user表可能不是很合适。

外，如果进一步支持其他多种第三方登录，那么这里需要重新设计。

### 2.1.3 行政区域设计
 
lumchine_region表保存了行政区域信息，包括省级、市级、县级三个等级，

原nideship.sql中存在region数据，但是lumchine.sql的region数据则来自
...
由lumchine_order表和lumchine_order_goods表保存。
...
所对应的后台服务方法是lumchine-wx-api模块的`WxOrderController.submit`方法。
...
所对应的后台服务方法是lumchine-wx-api模块的`WxOrderController.prepay`
...
所对应的后台服务方法是lumchine-wx-api模块的`WxOrderController.cancel`
...
对应的应该是lumchine-admin-api模块的系统定时任务的`OrderJob.checkOrderUnpaid`
...
所对应的后台服务方法是lumchine-wx-api模块的`WxOrderController.payNotify`
...
所对应的后台服务方法是lumchine-wx-api模块的`WxOrderController.refund`
...
所对应的后台服务方法是lumchine-admin-api模块的`AdminOrderController.refundConfirm`
...
所对应的后台服务方法是lumchine-admin-api模块的`AdminOrderController.ship`

* 301 -> 401

当用户收到商品以后，用户点击`收货确认`按钮，设置订单状态401，表示用户成功收货。

所对应的后台服务方法是lumchine-wx-api模块的`WxOrderController.confirm`
...
应该改为 lumchine-admin-api模块的系统定时任务`OrderJob.checkOrderUnconfirm`
...
在lumchine_order表中存在`comments`字段，表示有几个订单商品没有评价；
而在lumchine_order_goods表中存在`comment`字段，表示当前订单商品的评论ID。
...
评论表lumchine_comment保存评论相关的信息，其中最关键的是`type`字段和`value_id`字段。
...
团购是由团购规则表lumchine_groupon_rules和团购活动表lumchine_groupon组成。

管理员在管理后台对一些商品配置团购规则，保存在lumchine_groupon_rules表中。

用户在小商场中则看到团购规则给出的优惠信息。
接下来用户存在两种操作：
第一种是，用户开团，保存在lumchine_groupon中，用户主动分享商品团购页面给朋友；
第二张是，用户参团，也保存在lumchine_groupon中。

只有开团人数符合团购规则条件，创建的订单才会有效，否则管理员需要退款取消当前团购。

### 2.1.7 优惠券设计

优惠券由lumchine_coupon表和lumchine_coupon_user表组成：
* lumchine_coupon表，是优惠券基本信息及使用规则。
* lumchine_coupon_user表，是用户优惠券领取和使用的记录。
...
这里需要指出的是，lumchine_coupon表和lumchine_coupon_user表都有status字段。

lumchine_coupon表的status字段，包含以后三种状态：
* 正常可用，
* 已过期，
* 已下架，

lumchine_coupon_user表的status字段，包含以后三种状态：
* 未使用，
* 已使用，
* 已过期，
* 已下架，
...
除了以上表，数据库还存在其他一些业务表，例如专题表lumchine_topic，
但是都很直观，不需要多讨论。
...
lumchine-db模块是一个普通的Spring Boot应用，基于mybatis框架实现数据库访问操作，对外提供业务数据访问服务。

此外，lumchine-db最终是作为一个类库被其他模块所依赖使用，因此并不对外
...
3. 根据`table`, 自动生成三种代码:
   * src文件夹`com.lumchine.db.dao` 包内的Java代码
   * src文件夹`com.lumchine.db.domain` 包内的Java代码
   * resources文件夹`com.lumchine.db.domain.dao` 内的XML文件

以上三种代码即可封装对数据库的操作，开发者无需直接操作sql代码，
而是直接操作Java代码来完成对数据库的访问处理。

关于如何基于mybatis的Example代码来访问数据库，请查阅相关资料，
或者参考本模块`com.lumchine.db.service` 包内的Java代码。

当然，为了达到数据库访问效率，开发者也可以手动自定义mapper文件和对应的Java代码。
例如，当需要访问两个表的数据时，这里是在业务层通过Java代码遍历的形式来访问两个表，
也可以通过自定义的mapper文件来实现。

接下来，以`lumchine_brand`表举例说明如何自动生成代码：

1. mybatis generator插件会读取`table`标签

    ```
    <generatorConfiguration>
         <table tableName="lumchine_brand">
             <generatedKey column="id" sqlStatement="MySql" identity="true" />
         </table>
    </generatorConfiguration>
    ```
    
2. 自动生产src文件夹下domain包内的LumchineBrand.java类、LumchineBrandExample.java类、
    dao包内的LumchineBrandMapper.java接口和resources文件夹下dao包内的LumchineBrandMapper.xml文件。

3. 手动在service包内创建LumchineBrandService.java来对外提供具体的服务。
   ...
   ```java
    ...
    public class LumchineBrandService {
       ...
       private LumchineBrandMapper brandMapper;
        ...
        public List<LumchineBrand> query(int offset, int limit) {
...
1. dao代码
   ...
   在src文件夹`com.lumchine.db.domain` 包内的StatMapper.java代码定义了数据库访问的接口

2. domain代码
 
   如果希望数据库操作返回数据模型，可以在src文件夹`com.lumchine.db.domain` 包内创建相应代码。
   而这里统计功能是采用简化的`List<Map>`保存数据，没有定义domain代码。

3. XML文件

   在resources文件夹`com.lumchine.db.domain.dao` 内的StatMapper.xml文件则是实现真正的数据库访问操作。

4. service代码

   这里可以在`com.lumchine.db.service` 内定义一个StatServie.java代码，调用底层mapper代码，对外服务。
    ```
    @Service
...
通常业务层代码在src文件夹`com.lumchine.db.service` 包中。
...
2. 在mybatis generator配置文件中，配置需要的字段；
    ```
        <table tableName="lumchine_goods">
            <columnOverride column="gallery" javaType="java.lang.String[]"
                            typeHandler="JsonStringArrayTypeHandler"/>
        </table>
    ```
3. 使用mybatis generator自动生成代码，可以看到LumchineGoods的gallery是`String[]`类型。
...
1. 在数据库里面创建一个表，例如`lumchine_demo`:

    ```sql
    CREATE TABLE `lumchine`.`lumchine_demo` (
      `id` INT NOT NULL AUTO_INCREMENT,
      `name` VARCHAR(45) NULL,
      `address` VARCHAR(45) NULL,
      PRIMARY KEY (`id`));
      
    INSERT INTO `lumchine`.`lumchine_demo` (`id`, `name`, `address`) 
...
2. 在generatorConfig.xml中增加一个新的table标签
    ...
    ```
    <generatorConfiguration>
         <table tableName="lumchine_demo">
             <generatedKey column="id" sqlStatement="MySql" identity="true" />
         </table>
    </generatorConfiguration>
    ```
3. 双击mybatis generator插件，检查LumchineDemo.java类、LumchineDemoExample.java类、
   LumchineDemoMapper.java接口和LumchineDemoMapper.xml是否生产。
   
4. 在service里面新建LumchineDemoService.java类，

   ```java
    @Service
    public class LumchineDemoService {
       @Resource
       private LumchineDemoMapper demoMapper;

        public List<LumchineDemo> list() {
           LumchineDemoExample example = new LumchineDemoExample();
           return demoMapper.selectByExample(example);    
        }
    }
   ```

5. 可以在`src/test/java/com.lumchine.db`包里面创建LumchineDemoTest.java类,
    使用Junit进行测试。
    ...
    @SpringBootTest
    public class LumchineDemoTest {    
       @Autowired
       private LumchineDemoService demoService;
    
       @Test
       public void test() {    
        List<LumchineDemo> lumchineDemoList = demoService.list();
        Assert.assertTrue(lumchineDemoList.size() != 0);
       }
    
    }
    ```

6. 同样地，可以在Controller中使用LumchineDemoService来对外提供服务。
    ...
    public class DemoController {
       ...
       @RequestMapping("/list")
       public Object list(){    
           List<LumchineDemo> demoList = demoService.list();   
           return demoList;
       }
    }
    ```
...
lumchine-db模块中不涉及到事务管理，而是在其他后台服务模块中涉及。
但是其他后台服务模块因为依赖lumchine-db模块，因此这里列出。
...
* system
  ...
  通过lumchine-db模块的数据库访问，读取本项目系统配置信息。
...
注意
> 这里的util代码不会涉及具体业务，例如lumchine-db模块中存在一个
> OrderUtil类处理数据库中lumchine_order表的一些转换工作。
...
查看lumchine-all模块，代码仅仅只有一个Application类。

实际的原理是lumchine-all模块内的pom.xml文件：

1. 打包方式是`jar`，因此最后会打包可执行jar格式；
2. 对lumchine-wx-api模块和lumchine-admin-api模块依赖，
   因此打包时会作为依赖库而打包到lumchine-all模块的输出中；
3. 使用copy-resources插件，在打包时把lumchine-admin模块的dist
   文件夹拷贝到lumchine-all模块的static文件夹中；而这个文件夹
   正是Spring Boot应用的默认静态文件路径。
   
   注意：
   > 这个插件只是简单的拷贝操作；因此开发者应该在打包lumchine-all
   > 之前确保先编译lumchine-admin模块得到最终静态文件。
