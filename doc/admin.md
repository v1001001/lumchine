...
权限控制在数据库层面涉及到三个表`lumchine_admin`, `lumchine_role`和`lumchine_permission`：
* lumchine_admin表中存在roleId字段，保存角色ID数组；
* lumchine_role表记录角色名称和角色介绍；
* lumchine_permission表记录角色所用于的权限值。
...
后端权限基于shiro来实现，相关代码见lumchine-admin-api模块。
...
public class AdminAdController {
    ...
    public Object list(String name, String content,
        ...
        List<LumchineAd> adList = adService.querySelective(name, content, page, limit, sort, order);
...
lumchine-admin模块的代码基于[vue-element-admin](https://github.com/PanJiaChen/vue-element-admin)
