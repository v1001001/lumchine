...
* 小商城前端，即lumchine-wx模块和renard-wx模块
  * 微信小程序
* 小商城后端，即lumchine-wx-api模块
  * Spring Boot 2.x
  * Spring MVC
...
1. 在lumchine-core模块的src/main/resources的application-core.yml资源文件中设置
    ```
    lumchine
        wx
            app-id: 开发者申请的app-id
...
2. 在lumchine-wx模块的project.config.json文件中设置
...
4. 建议开发者关闭当前项目或者直接关闭微信开发者工具，重新打开（因为此时lumchine-wx模块的appid可能未更新）。
   编译运行，尝试微信登录
...
1. 在lumchine-core-api模块的src/main/resources的application-core.yml资源文件中设置
...
2. 在lumchine-core-api模块的src/main/resources的application-core.yml资源文件中设置
...
小商城后端服务每一次请求都会检测是否存在HTTP头部域`X-Lumchine-Token`。
...
注意
> lumchine-wx模块代码基于nideshop-mini-program的commit版本[acbf6276eb27abc6a48887cddd223d7261f0088e](https://github.com/tumobi/nideshop-mini-program/commit/acbf6276eb27abc6a48887cddd223d7261f0088e)。
> 由于改动变化较大，因此之后lumchine-wx将独立开发，不会合并nideshop-mini-program的更新。
...
lumchine-wx模块采用storage来存储一些数据，以及支持组件间数据通信。
...
renard-wx是另外一个小程序前端，其后端API也是lumchine-wx-api。
