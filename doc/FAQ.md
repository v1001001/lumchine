...
这里主要是指lumchine-wx-api、lumchine-wx和renard-wx三个模块的相关问题。
...
2. 开发者在lumchine-wx、renard-wx和lumchine-core模块的appid信息设置成自己申请的信息。
...
微信开发者工具中修改appid以后，请关闭lumchine-wx项目或者微信开发者工具，重新启动导入lumchine-wx。
...
```
lumchine.wx.app-id=
lumchine.wx.app-secret=
lumchine.wx.mch-id=
lumchine.wx.mch-key=
lumchine.wx.notify-url=
```
...
这里主要是指lumchine-admin-api和lumchine-admin两个模块的相关问题。
...
2. 其次，需要明白报错的地方，是lumchine-admin/src/utils/request.js文件中;
...
lumchine-db模块的application-db.yaml资源文件中reasonable是true
...
这里主要是指lumchine-db、lumchine-core和lumchine-all模块三个模块的相关问题。
...
```
org.apache.ibatis.binding.BindingException: Invalid bound statement (not found): com.lumchine.db.dao.XXXX
```
...
The error may exist in org/v1001001/lumchine/db/dao/LumchineSystemMapper.xml
The error may involve LumchineSystemMapper.selectByExample-Inline
The error occurred while setting parameters
SQL: select id, key_name, key_value, add_time, update_time, deleted from lumchine_system
...
开发者直接（或使用Navicat）运行lumchine_schema.sql时运行失败。
...
首先，请开发者请直接打开lumchine_schema.sql文件，可以看到
```
...
因此，如果开发者运行lumchine_schema.sql失败，开发者可以自行使用
相关SQL命令或者使用SQL工具创建数据库、用户和分配权限工作。

此外实际上，**开发者也不应该在部署或者上线阶段运行lumchine_schema.sql**
...
应该是lumchine-admin模块和lumchine-vue模块的node_modules文件夹导致的。
node_modules是lumchine-admin和lumchine-vue模块所依赖的项目库，可能有近200M的文件。
而IDEA如果没有设置，则可能尝试对该文件夹进行解析索引，从而导致卡断。

解决方案：

1. 先关闭IDEA，然后删除lumchine-admin和lumchine-vue模块内的node_modules文件夹；
2. 然后分别创建空的node_modules文件夹；
3. 重新打开IDEA，分别设置lumchine-admin模块和lumchine-vue模块的node_modules文件夹Excluded状态。
