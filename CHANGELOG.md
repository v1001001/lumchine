...
*2018-07-30*，删除os模块，小程序完善
  ...
  * `小商场`支持商品分享
  * `小商场`物流显示
  * `小商场`优化商品购买样式，感谢[usgeek](https://github.com/v1001001/lumchine/pull/31) 
  * `小商场`支持客服、关于页面、手机绑定，感谢[usgeek](https://github.com/v1001001/lumchine/pull/31) 
  * `小商场`支持账号注册和密码找回时短信验证码
  * `管理后台`页面查询时默认基于创建时间排序
  * `基础系统`支持物流追踪  
  * `基础系统`支持阿里云存储，感谢[usgeek](https://github.com/v1001001/lumchine/pull/31)  
  * `项目`删除os模块，相关功能迁移至wx-api模块和admin-api模块
  * `项目`从默认单机多服务方案调整至默认单机单服务方案
  * `项目`暂时取消docker支持和.gitlab-ci.yml支持
  * `项目`数据库添加lumchine_system表，存储系统配置信息
  * `项目`取消swagger支持
...
*2018-07-16*，数据库再次简化，同时支持短信提醒、邮件提醒、腾讯对象存储服务
  ...
  * `管理后台`多个页面完善页面效果
  * `管理后台`支持商品上架和商品编辑
  * `基础系统`支持腾讯云短信提醒和邮件提醒，感谢[Menethil](https://github.com/v1001001/lumchine/pull/23)
  * `基础系统`支持腾讯对象存储，感谢[Menethil](https://github.com/v1001001/lumchine/pull/24)
...
*2018-05-11*，项目支持微信支付和修复小程序的一些BUG
  ...
  * `小商场`支持微信支付
  * `管理后台`支持微信支付
  * `基础系统`数据库lumchine_collect的id设置自增
  * `基础系统`数据库删除region相关的四个临时表
  * `基础系统`lumchine-core模块swagger2配置，支持swagger文档
  * `项目`多模块maven结构重新设计
  * `项目`文档整理部署方案和上线方案
...
*2018-04-21*，项目结构调整，增加了两个模块
  ...
  * `管理后台`专题内容支持富文本编辑
  * `基础系统`lumchine-os-api的链接从`storage/index/index`调整至`os/index/index`
  * `基础系统`lumchine-os-api支持浏览器显示。
  * `基础系统`新增lumchine-core模块，综合了lumchine-os-api、lumchine-wx-api
     和lumchine-admin-api的共性代码
  * `基础系统`新增lumchine-all模块作为包裹模块，支持三个后端服务和静态文件
     打包成一个war项目包
...
* `基础系统`lumchine-db模块不支持数据物理删除，删除则设置`deleted`，而查询则过滤`deleted`
