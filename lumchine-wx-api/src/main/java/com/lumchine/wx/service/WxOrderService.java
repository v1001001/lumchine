...
import com.lumchine.core.express.ExpressService;
import com.lumchine.core.express.dao.ExpressInfo;
import com.lumchine.core.notify.NotifyService;
import com.lumchine.core.notify.NotifyType;
import com.lumchine.core.qcode.QCodeService;
import com.lumchine.core.system.SystemConfig;
import com.lumchine.core.util.DateTimeUtil;
import com.lumchine.core.util.IpUtil;
...
import com.lumchine.db.util.OrderHandleOption;
...
public class WxOrderService {
    ...
    @Autowired
    private LumchineCommentService commentService;
    @Autowired
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<LumchineOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (LumchineOrder o : orderList) {
            ...
            LumchineGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<LumchineOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        LumchineOrder order = orderService.findById(orderId);
        if (null == order) {
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            LumchineGrouponRules rules = grouponRulesService.queryById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
        ...
        // 收货地址
        LumchineAddress checkedAddress = addressService.query(userId, addressId);
        ...
        for (LumchineCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            LumchineCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        LumchineOrder order = null;
        // 订单
        order = new LumchineOrder();
        ...
        // 添加订单商品表项
        for (LumchineCart cartGoods : checkedGoodsList) {
            // 订单商品
            LumchineOrderGoods orderGoods = new LumchineOrderGoods();
        ...
        for (LumchineCart checkGoods : checkedGoodsList) {
            ...
            LumchineGoodsProduct product = productService.findById(productId);
        ...
        // 如果使用了优惠券，设置优惠券使用状态
        if (couponId != 0 && couponId != -1) {
            LumchineCouponUser couponUser = couponUserService.queryOne(userId, couponId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            LumchineGroupon groupon = new LumchineGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                LumchineGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        LumchineOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        LumchineGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            LumchineGrouponRules grouponRules = grouponRulesService.queryById(groupon.getRulesId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<LumchineOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        LumchineOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        LumchineOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        LumchineComment comment = new LumchineComment();
