...
import com.lumchine.db.service.LumchineSearchHistoryService;
...
public class WxSearchController {
    ...
    @Autowired
    private LumchineKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        LumchineKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<LumchineKeyword> hotKeywordList = keywordsService.queryHots();

        List<LumchineSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<LumchineKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (LumchineKeyword key : keywordsList) {
