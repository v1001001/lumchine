...
public class WxHomeController {
    ...
    @Autowired
    private LumchineAdService adService;
    ...
    private List<Map> getCategoryList() {
        ...
        List<LumchineCategory> catL1List = categoryService.queryL1WithoutRecommend(0, SystemConfig.getCatlogListLimit());
        for (LumchineCategory catL1 : catL1List) {
            List<LumchineCategory> catL2List = categoryService.queryByPid(catL1.getId());
            List<Integer> l2List = new ArrayList<>();
            for (LumchineCategory catL2 : catL2List) {
                l2List.add(catL2.getId());
            }

            List<LumchineGoods> categoryGoods;
            if (l2List.size() == 0) {
