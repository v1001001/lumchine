...
import com.lumchine.admin.service.LogHelper;
...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<LumchineAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(LumchineAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody LumchineAdmin admin) {
        ...
        List<LumchineAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        LumchineAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody LumchineAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody LumchineAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        LumchineAdmin currentAdmin = (LumchineAdmin) currentUser.getPrincipal();
